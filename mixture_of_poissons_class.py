...
class PoissonMixture():
    ...
    def errors(self, probs, lambdas):
        """
        Throws an error if the following does not hold:
            len(self.probs) == len(self.lambdas) == self.K
        """
        if len(probs) != len(lambdas):
            raise ValueError('len(probs) (',len(probs),') and len(lambdas) (',len(lambdas),') should be the same.')
        if len(probs) != self.K:
            raise ValueError('len(probs) (', len(probs), ') and K (', self.K, ') should be the same.')
